...
# ── CONFIG ────────────────────────────────────────────────────────────────────
WEIGHTS_PATH = "SAR/models/best_denoising_model_Inception_Full.pth"
...
RESIZE       = (512, 512)
NUM_SAMPLES  = 5   # how many images to visualise
...
class Inception(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.kernel_3x3 = nn.Sequential(
            nn.Conv2d(in_channels, bc, 1), nn.BatchNorm2d(bc), nn.ReLU(),
            nn.Conv2d(bc, bc, 3, padding=1), nn.BatchNorm2d(bc), nn.ReLU())
        self.kernel_5x5 = nn.Sequential(
            nn.Conv2d(in_channels, bc, 1), nn.BatchNorm2d(bc), nn.ReLU(),
            nn.Conv2d(bc, bc, 5, padding=2), nn.BatchNorm2d(bc), nn.ReLU())
        self.pooling = nn.Sequential(
            nn.MaxPool2d(3, stride=1, padding=1),
            nn.Conv2d(in_channels, bc, 1), nn.BatchNorm2d(bc), nn.ReLU())

    def forward(self, x):
        return torch.cat([self.kernel_1x1(x), self.kernel_3x3(x),
                          self.kernel_5x5(x), self.pooling(x)], dim=1)
...
def visualise():
    ...
    for fname in files:
        ...
        gt = np.array(Image.open(gtruth_path).convert('L').resize(
                      (RESIZE[1], RESIZE[0]), Image.BILINEAR), dtype=np.uint8)

        # diff map: absolute error between denoised and ground truth
        diff = cv2.applyColorMap(
            cv2.normalize(np.abs(denoised_np.astype(np.int16) - gt.astype(np.int16))
                          .astype(np.uint8), None, 0, 255, cv2.NORM_MINMAX),
            cv2.COLORMAP_HOT)

        # stack: Noisy | Denoised | Ground Truth | Diff
        row = cv2.hconcat([
